...
export class DataService {
  ...
  static getOrderManifest(orderId: string) {
    const order = this.getOrders().find((o: any) => o.id === orderId);
    if (!order) return [];
    return (order.items || []).map((item: any) => ({
      ...item,
      sku: item.sku || `SKU-${Math.floor(Math.random() * 9999)}`,
      image: item.image || 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?q=80&w=200'
    }));
  }
