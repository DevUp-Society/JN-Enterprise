...
const ProductMatrix: React.FC<ProductMatrixProps> = ({ product, onAddToCart }) => {
  ...
  return (
    <div className="w-full bg-slate-50/80 rounded-2xl p-5 border border-slate-200/60 shadow-sm">
      <div className="grid grid-cols-4 gap-3 mb-5">
        {SIZES.map(size => (
          <div key={size} className="flex flex-col items-center gap-3">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{size}</span>
            <div className="flex flex-col items-center bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm hover:border-indigo-200 transition-colors">
              <button 
                onClick={() => updateQuantity(size, 1)}
                className="p-2 hover:bg-slate-50 text-indigo-600 transition-colors border-b border-slate-100"
              >
              ...
              <input 
                ...
                placeholder="0"
                className="w-12 text-center py-1.5 text-sm font-black text-slate-900 outline-none bg-transparent [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none focus:bg-indigo-50/30 transition-colors"
      ...
      <div className="flex items-center justify-between gap-5 pt-3 border-t border-slate-200/80 mt-1">
        ...
        <button
          ...
          className="flex-grow bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-200 text-white disabled:text-slate-400 py-3.5 px-5 rounded-2xl font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-3 transition-all active:scale-95 shadow-lg shadow-indigo-500/10 hover:shadow-indigo-500/20"
        >
