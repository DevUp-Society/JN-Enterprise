...
export default function HighDensityChart({ data, color = '#C6AD8F', height = 300 }: HighDensityChartProps) {
  ...
  return (
    <div className="relative w-full overflow-visible" style={{ height }}>
      ...
      >
        <defs>
          <linearGradient id="spline-gradient-v3" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#C6AD8F" stopOpacity="0.25" />
            <stop offset="100%" stopColor="#C6AD8F" stopOpacity="0" />
          </linearGradient>
        </defs>
      ...
      {hoveredIndex !== null && (
        <div 
          className="absolute z-20 pointer-events-none bg-[#1E3A8A] border border-white/20 shadow-2xl p-4 min-w-[160px]"
          style={{ 
        ...
        >
          <div className="space-y-1">
             <p className="text-[7px] font-black text-white/40 uppercase tracking-[0.3em] italic">DATAPOINT_SYNC_</p>
          ...
          <div className="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-t-[6px] border-t-[#1E3A8A]" />
        </div>
      )}
    </div>
  );
