                ...
                <div className="grid grid-cols-1 gap-4">
                   {presets.map(p => (
                     <button 
                        ...
                        className={`h-16 border border-primary/10 flex items-center justify-center text-[10px] font-black uppercase tracking-[0.2em] transition-all ${activePreset === p.id ? 'bg-[#000000] text-white border-[#000000]' : 'hover:bg-primary hover:text-white'}`}
                     >
                        {p.label}
                     </button>
                   ))}
       ...
       <div className="flex items-center gap-4 px-4 bg-bone/30 dark:bg-white/5 py-2">
          ...
          <div className="flex items-center gap-4">
             <div className="space-y-0 text-left">
                ...
                <p className="text-[12px] font-black text-black dark:text-black leading-none">{formatDate(startDate)}</p>
             </div>
             ...
             <div className="space-y-0 text-left">
                ...
                <p className="text-[12px] font-black text-black dark:text-black leading-none">{formatDate(endDate)}</p>
             </div>
          </div>
       </div>
       ...
       <div className="flex items-center gap-1 border-l border-primary/10 pl-4">
          {presets.map(p => (
            <button 
               key={p.label}
               onClick={() => handlePreset(p.id, p.getRange)}
               className={`px-3 h-8 border text-[9px] font-black uppercase tracking-widest transition-all ${activePreset === p.id ? 'bg-[#000000] text-white border-[#000000]' : 'border-primary/5 text-primary/40 dark:text-black/40 hover:bg-primary hover:text-white hover:border-primary'}`}
            >
               {p.label}
            </button>
          ))}
          ...
          <div className="relative">
             <button 
                ...
                className={`flex items-center gap-2 px-3 h-8 border ${isCustomMode || (activePreset === null) ? 'bg-[#000000] text-white border-[#000000]' : 'border-primary/5 text-primary/40'} text-[9px] font-black uppercase tracking-widest hover:bg-[#000000] hover:text-white transition-all`}
             >
                <Calendar size={12} />
                CUSTOM
             </button>
                   ...
                   >
                      <div className="grid grid-cols-1 gap-6">
                         <div className="space-y-2">
                            <label className="text-[9px] font-black uppercase tracking-widest text-primary/40 dark:text-black/40">START DATE</label>
                            <input ref={startInputRef} type="date" className="w-full h-12 px-4 bg-bone/50 dark:bg-dark border border-primary/10 text-[11px] font-black outline-none focus:border-[#000000] transition-colors dark:text-white" />
                         </div>
                         <div className="space-y-2">
                            <label className="text-[9px] font-black uppercase tracking-widest text-primary/40 dark:text-black/40">END DATE</label>
                            <input ref={endInputRef} type="date" className="w-full h-12 px-4 bg-bone/50 dark:bg-dark border border-primary/10 text-[11px] font-black outline-none focus:border-[#000000] transition-colors dark:text-white" />
                         </div>
                      </div>
                      <button 
                         onClick={handleCustomSubmit}
                         className="w-full h-12 bg-[#000000] text-white text-[10px] font-black uppercase tracking-[0.2em] hover:bg-black transition-colors"
                      >
                         ENTER
                      </button>
                   </motion.div>
