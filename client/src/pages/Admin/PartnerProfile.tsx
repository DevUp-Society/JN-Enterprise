...
export default function PartnerProfile() {
  ...
  const stats = [
    ...
    { label: 'Outstanding Balance', value: '₹' + (partner.lifetimeRevenue * 0.15).toLocaleString(), icon: CreditCard, color: 'text-red-500' }
  ];
                   ...
                   >
                      ...
                      <td className="p-8">
                         <div className="flex flex-col items-center gap-2">
                            <div className={`px-4 h-6 border flex items-center justify-center text-[8px] font-black uppercase tracking-widest ${order.status === 'DELIVERED' ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-gold/10 text-gold border-gold/20'}`}>
                               {order.status}
                            </div>
                         </div>
                      </td>
